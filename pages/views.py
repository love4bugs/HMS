...
from pages.forms import BookingForm, CheckoutForm, RoomForm
...
def book_view(request):
    ...
    content = {"form": form, "qs": queryset}

    if request.method == "POST":
        form = BookingForm(request.POST)
        if form.is_valid():
            Room.objects.filter(room_no=form.cleaned_data["room_no"]).update(
                empty=False
            )
            form.save()
            return redirect("/home")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            Checkout.objects.filter(pk=instance.pk).update(
                consumer=Costumer.objects.filter(
                    room_no=instance.room_no, phone=instance.phone, active=True
                )[0]
            )
...
def resturant_bill(request):
    ...
    if request.method == "POST":
        ...
        if room_id:
            try:
                obj_consumer = Costumer.objects.filter(
                    room_no=int(room_id), active=True
                )[0]
...
def resturant_reciept(request, my_order):
    """Resturant Reciept"""
    ...
    if query_set[0].consumer is not None and (
        query_set[0].consumer.package == "P" or query_set[0].consumer.package == "G"
    ):
        total = 0
    content = {
        "bill": item_list,
...
def pool_reciept(request, my_usage_id):
    """Pool Reciept"""
...
def checkout_bill(request, timestamp):
    """Checkout Bill"""
    obj = Checkout.objects.filter(phone=timestamp, paid=False)[0]
    obj1 = Costumer.objects.filter(phone=timestamp, active=True)[0]
    obj2 = Room.objects.get(room_no=obj.room_no)

    query_set = RestaurantParchase.objects.filter(consumer=obj1, paid=False).order_by(
        "order_id"
    )
    resturant_obj = []
    try:
    ...
    gym_obj.append(order)
    gym_cost = sum(
        [obj.time_spent_in_hours * obj.gym.cost_per_hour for obj in query_set1]
    )

    query_set2 = PoolUsage.objects.filter(consumer=obj1, paid=False).order_by(
        "usage_id"
    )
    pool_obj = []
    try:
    ...
    pool_obj.append(order)
    pool_cost = sum(
        [obj.time_spent_in_hours * obj.pool.cost_per_hour for obj in query_set2]
    )
    ...
    total = (
        (time.days + 1) * obj2.room_cost
        - obj1.advance
        + obj1.get_package_cost()
        + resturant_cost
        + gym_cost
        + pool_cost
    )
    ...
    Checkout.objects.filter(phone=timestamp).update(paid=True)
...
def list_resturant_bills(request):
    """List Resturant Bills"""
